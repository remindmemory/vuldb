...
func Fixbehind(SInglestring string) string {
	...
	for i := 1; i <= len(string_slice)-1; i++ {
		string_slice[i] = "0"
	}

	Number, _ := strconv.Atoi(string_slice[0])
	Number++
	string_slice[0] = strconv.Itoa(Number)
...
func FixRubybehind(SInglestring string) string {
	var finalstr string
	//strings.LastIndex(SInglestring, ".")
...
func FixPatchedversions(Patchedversions []string) []string {
	...
	for _, Patchversionvalue := range Patchedversions {
		...
		if strings.Index(Patchversionvalue, "^") == -1 && strings.Index(Patchversionvalue, "~") == -1 {
			//	strings.Replace(Patchversionvalue, " ", "", -1)
			Fixedversion = append(Fixedversion, Patchversionvalue)
			continue
		}
		if strings.Index(Patchversionvalue, "^") != -1 { //只找到了悲观没找到逗号
		...
		if strings.Index(Patchversionvalue, "~") != -1 { //只找到了悲观没找到逗号
			string_slice := strings.Split(Patchversionvalue, "~")
			SInglestring = ">=" + string_slice[1] + ",<" //首先确定好前半段
			SInglestring = SInglestring + FixRubybehind(string_slice[1])
		}
...
func MakeArr(PicesPatched []string) []string {
	var StrArr []string
	for _, singlestr := range PicesPatched {
		StrArr = append(StrArr, singlestr)
	}
	return StrArr
}
func IfPathIsCore(dirpath string, file os.FileInfo) {
	var Notedata NodesCoredata
	content, err := ReadFile(dirpath + "\\" + file.Name()) //读取nvd文件获取数据
	err = json.Unmarshal(content, &Notedata)
	if err != nil {
		fmt.Println(err)
	}
	if len(Notedata.Cve) < 1 {
		return
	}
	Notedata.Patched = strings.Replace(Notedata.Patched, " ", "", -1) //去掉所有空格
	PicesPatched := strings.Split(Notedata.Patched, "||")             //按照||分割
	Fixedversion := FixPatchedversions(MakeArr(PicesPatched))         //对版本符号进行处理
	if err != nil {
		fmt.Println(err)
		tracefile(file.Name()+"\n", "E:\\errname.json")
	}

	for _, EachCveid := range Notedata.Cve {
		str := "\"" + dirpath +"\\\\"+ file.Name() + "\t\t" + EachCveid + "\"\n"
		tracefile(str, "E:\\Nodepath\\Cveid.json")
		parseJsonData(EachCveid, Fixedversion, "E:\\Nodepath\\Nodedata.json")
	}
}

func IfPathIsNmp(dirpath string, file os.FileInfo) {
	var Notedata NodesNpmdata
	content, err := ReadFile(dirpath + "\\" + file.Name()) //读取nvd文件获取数据
	err = json.Unmarshal(content, &Notedata)
	if err != nil {
		fmt.Println(err)
	}
	if len(Notedata.Cves) < 1 {
		return
	}
	Notedata.PatchedVersions = strings.Replace(Notedata.PatchedVersions, " ", "", -1) //去掉所有空格
	PicesPatched := strings.Split(Notedata.PatchedVersions, "||")                     //按照||分割
	Fixedversion := FixPatchedversions(MakeArr(PicesPatched))                         //对版本符号进行处理
	if err != nil {
		fmt.Println(err)
	...
	for _, EachCveid := range Notedata.Cves {
		str := "\"" + dirpath + "\\\\"+file.Name() + "\t\t" + EachCveid + "\"\n"
		tracefile(str, "E:\\Nodepath\\Cveid.json")
		parseJsonData(EachCveid, Fixedversion, "E:\\Nodepath\\Nodedata.json")
	}
...
func SearchAllDir(dirpath string, depth int) {
	...
	for _, file := range files {
		if file.IsDir() {
			...
			if strings.Index(dirpath, "core") != -1 {
				IfPathIsCore(dirpath, file)
			}
			if strings.Index(dirpath, "npm") != -1 {
				IfPathIsNmp(dirpath, file)
			}
...
func main() {
	...
	Pyth_path := "E:\\Nodepath"
	SearchAllDir(Pyth_path, 0)
	CustomArr := Fixjsonfile("E:\\Nodepath\\Nodedata.json")
	writeArrfile(CustomArr, "E:\\Nodepath\\Nodefixeddata.json")
