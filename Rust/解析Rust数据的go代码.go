...
func SearchAllDir(dirpath string, depth int) {
	...
	for _, file := range files {
		if file.IsDir() {
			...
			_, err = toml.DecodeFile(dirpath+"\\"+file.Name(), &Rustdata)
			if err != nil {
				fmt.Println(err)
			}
			parseJsonData(Rustdata.Advisory.ID,Rustdata.Versions.Patched , "E:\\Rust\\Rustdata.json")
		}
	}
	return
}
...
func main() {
	...
	Pyth_path := "E:\\Rust\\advisory-db-master\\rust"
